...
| `bin/sd-watchdog` | (all-in-one) Self-heal: probes server + kiosk + page liveness/advance. A browser-level fault (server reachable) escalates only to relaunch-kiosk, then gives up and observes; only the server being genuinely unreachable escalates further, to restart-container then reboot (capped 3-per-6h, never within 10 min of boot) — a reboot never fixed a wedged page (ADR-121). Pauses entirely while an Admin → Devices action is in flight. Ships in `WATCHDOG=observe` (logs only) until you set `enforce`; run by `sd-watchdog.timer` every 60 s. |
...
| `bin/sd-update` | (all-in-one) Root host helper for every GUI action — 16 whitelisted actions matched in a `case`, never evaluated; triggered by `sd-update.path`. Writes `status.json`, persists `last-update.log`. A request whose `requested_at` predates this boot (a reboot happened before it finished, ADR-121) is reported as interrupted and consumed rather than silently re-run. |
